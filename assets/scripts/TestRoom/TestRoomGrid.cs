...
public class TestRoomGrid : MonoBehaviour
{
    ...
    private Color mainColor = new Color(215f,88f,44f,0.9f);
        ...
        if(lastScroll + scrollRate < Time.time)
        {
            if(Input.GetKey (KeyCode.A))
            {
                plane.transform.position  = new Vector3(plane.transform.position.x, 
                                                        plane.transform.position.y+smallStep, 
    ...
    void OnPostRender()
    {
        ...
        if (showMain) {
            ...
            //y axis lines
            for (float i = 0; i <= gridSizeZ; i += largeStep) {
                for (float k = 0; k <= gridSizeZ; k += largeStep) {
                    GL.Vertex3 (startX + k, startY + offsetY, startZ + i);
                    GL.Vertex3 (startX + k, gridSizeY + offsetY, startZ + i);
                }
            }
